...
public class UseTimeAop {
    ...
    @Around("product() || folder() || naver()") // 수행되기전후
    public Object execute(ProceedingJoinPoint joinPoint) throws Throwable {
        ...
        try {
            // 핵심기능 수행
            Object output = joinPoint.proceed(); // ex) productCreate() 실행
            return output;
        } finally {
            // 측정 종료 시간
            ...
            // Security Context 객체 뽑아온다. ->인증객체를 뽑아온다.
            if (auth != null && auth.getPrincipal().getClass() == UserDetailsImpl.class) {
                // 로그인 회원 정보
                UserDetailsImpl userDetails = (UserDetailsImpl) auth.getPrincipal(); // Casting
